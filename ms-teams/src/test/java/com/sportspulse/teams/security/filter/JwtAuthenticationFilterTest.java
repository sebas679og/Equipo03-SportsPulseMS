...
class JwtAuthenticationFilterTest {
    ...
    void doFilterInternal_shouldReturn401_whenAuthHeaderIsMissing() throws Exception {
        ...
        verify(errorWriter).write(response, HttpStatus.UNAUTHORIZED,
                Errors.Code.MISSING_TOKEN, Errors.Message.MISSING_TOKEN);
    ...
    void doFilterInternal_shouldReturn401_whenTokenIsInvalid() throws Exception {
        ...
        when(authClient.validate(token))
                .thenReturn(new ValidateResponse(false, null, null, null));

        filter.doFilterInternal(request, response, filterChain);

        verify(errorWriter).write(response, HttpStatus.UNAUTHORIZED,
                Errors.Code.INVALID_TOKEN, Errors.Message.INVALID_TOKEN);
        verify(filterChain, never()).doFilter(any(), any());
    }
    ...
    void doFilterInternal_shouldReturn401_whenFeignThrowsException() throws Exception {
        ...
        verify(errorWriter).write(response, HttpStatus.UNAUTHORIZED,
                Errors.Code.TOKEN_VALIDATION_ERROR, Errors.Message.TOKEN_VALIDATION_ERROR);
        verify(filterChain, never()).doFilter(any(), any());
    }
}