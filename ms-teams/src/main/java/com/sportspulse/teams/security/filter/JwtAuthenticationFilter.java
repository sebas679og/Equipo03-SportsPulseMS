 ...
 *
 * <p>Extracts the Bearer token from the {@code Authorization} header, delegates
 * validation to {@link AuthClient}, and populates the {@link SecurityContextHolder}
 * on success. Returns a structured {@link ErrorResponse} on failure.
 */
@Component
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        if (authHeader == null || !authHeader.startsWith(BEARER_PREFIX)) {
            errorWriter.write(response, HttpStatus.UNAUTHORIZED, Errors.Code.MISSING_TOKEN, Errors.Message.MISSING_TOKEN);
            return;
        }
        ...
        try {
            ...
            if (!validateResponse.valid()) {
                errorWriter.write(response, HttpStatus.UNAUTHORIZED ,Errors.Code.INVALID_TOKEN, Errors.Message.INVALID_TOKEN);
                return;
            }
            ...
            errorWriter.write(response, HttpStatus.UNAUTHORIZED,Errors.Code.TOKEN_VALIDATION_ERROR, Errors.Message.TOKEN_VALIDATION_ERROR);
        }
    }
}
