...
public class TeamController {
  ...
  @Operation(
      summary = "Equipment query by league and season",
      description =
          """
                     Returns a list of teams and their stadium.
                     """,
      security = {
        @SecurityRequirement(name = "BearerAuth"),
      })
  @ApiResponses({
    @ApiResponse(
        ...
        content =
            @Content(
                mediaType = MediaType.APPLICATION_JSON_VALUE,
                schema = @Schema(implementation = DataLeagueSeasonResponse.class))),
    @ApiResponse(
        responseCode = "400",
        description = "Validation error - Invalid path parameter",
        content =
  ...
  @GetMapping(ApiPaths.Teams.TEAMS)
  public ResponseEntity<DataLeagueSeasonResponse> getTeamsByLeagueAndSeason(
      @Valid @ModelAttribute LeagueAndSeasonRequest request) {
    return ResponseEntity.status(HttpStatus.OK)
        .body(teamService.getTeamLeagueSeasonByTeamId(request));
  }
}
